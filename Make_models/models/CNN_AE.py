...
class MS_CNN_AE():
    ...
    def conf_model(self):
        ...
        x = Conv2D(
            self.config.model.chanel_nm[2], (3, 3), padding='same')(x)
        x = BatchNormalization()(x)
        encoded = Activation(self.config.model.act)(x)
        x = Conv2D(self.config.model.chanel_nm[2], (3, 3), padding='same')(
            encoded)
        x = BatchNormalization()(x)
...
class MS_FC_CNN_AE():
    ...
    def conf_model(self):
        ...
        x = Add()([conv1, conv2, conv3])
        x = Reshape([72])(x)
        for i in range(self.config.model.fc_num):
            x = Dense(self.config.model.uni_num[i])(x)
            x = BatchNormalization()(x)
            x = Activation(self.config.model.act)(x)
        encoded = Dense(self.config.model.encoded_size)(x)
        x = BatchNormalization()(encoded)
        x = Activation(self.config.model.act)(x)
        for i in reversed(range(self.config.model.fc_num)):
            x = Dense(self.config.model.uni_num[i])(x)
            x = BatchNormalization()(x)
            x = Activation(self.config.model.act)(x)
        x = Dense(72)(x)
